...
def generator_precipitation_mask(ruta_origen, ruta_destino):
    ...
    precipitation_mask_red,  precipitation_mask_purple = extractor_precipitation(img)
    
    precipitation_mask_red_aux = precipitation_mask_red.copy()
    precipitation_mask_purple_aux = precipitation_mask_purple.copy()
    ...
    return precipitation_mask_red_aux, precipitation_mask_purple_aux, img, name_img
...
def etiquetar_resultados_hydro(resultados_red, resultados_purple, imagen_rgb):
    """
    Etiqueta una imagen con los resultados de la función 'calcular_interseccion_carpeta'.
    ...
    if not resultados_red:
        print("No hay lluvias intensas")
        return imagen_rgb
    
    # Etiquetar cada imagen en los resultados
    for ruta, resultado in resultados_red.items():
        # Obtener el nombre de la imagen sin la extensión
        nombre = os.path.splitext(os.path.basename(ruta))[0]
    ...
    if not resultados_purple:
        print("No hay lluvias extremadamente intensas")
        return imagen_rgb
    
    # Etiquetar cada imagen en los resultados
    for ruta, resultado in resultados_purple.items():
        # Obtener el nombre de la imagen sin la extensión
        nombre = os.path.splitext(os.path.basename(ruta))[0]
...
def deteccion_cloud(destino , ruta_clouds_mask, ruta_departamentos_mask, save_path, treshold):
...
def deteccion_precipitation(destino , ruta_precipitation_mask, ruta_departamentos_mask, save_path, treshold):
    ...
    #obtenemos imagen con intersecciones localizadas
    new_img = etiquetar_resultados_hydro(array_lluvia_red, array_lluvia_morado, imagen_rgb)
